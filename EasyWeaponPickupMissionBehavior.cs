    ...
    public class EasyWeaponPickupMissionBehavior : MissionLogic
    {
            ...
            try
            {
                ...
                if (agentEquipmentControllerView == null)
                {
                    InformationManager.DisplayMessage(new InformationMessage(
                        new TextObject("{=c51D173a44}Unable to find MissionGauntletMainAgentEquipmentControllerView! easy pickup will temporary disabled for this scene!").ToString(),
                        Colors.Red
                     ));
                }
                else
        ...
        public override void OnFocusGained(Agent agent, IFocusable focusableObject, bool isInteractable)
        {
            ...
            try
            {
                ...
                if (EasyWeaponPickupConfig.Instance.DebugEnabled)
                {
                    InformationManager.DisplayMessage(new InformationMessage(new TextObject("{=1c41cDf6A9}pickup disabled").ToString()));
                }
        ...
        public override void OnFocusLost(Agent agent, IFocusable focusableObject)
        {
            ...
            if (EasyWeaponPickupConfig.Instance.DebugEnabled)
            {
                InformationManager.DisplayMessage(new InformationMessage(new TextObject("{=301715F362}pickup enabled").ToString()));
            }
        }
                        ...
                        {
                            {
                                InformationManager.DisplayMessage(new InformationMessage(
                                    new TextObject(
                                        "{=86907d5377}equip / use : {itemToUse} distance {distance} height : {height}",
                                        new Dictionary<string, object>() {
                                            { "itemToUse", itemToUse.ToString() },
                                            { "distance", itemToUse.GameEntity.GlobalPosition.AsVec2.DistanceSquared(Agent.Main.Position.AsVec2) },
                                            { "height", Math.Abs(itemToUse.GameEntity.GlobalPosition.Z - Agent.Main.Position.Z) }
                                        }
                                     ).ToString()
                                 ));
                            }
        ...
        private UsableMissionObject GetNearestDroppedItem(HashSet<ItemObject.ItemTypeEnum> pickableItem, HashSet<WeaponClass> allowedAmmoClass)
        {
            ...
            if (EasyWeaponPickupConfig.Instance.DebugEnabled)
            {
                InformationManager.DisplayMessage(new InformationMessage(new TextObject("{=AEd61b7E1b}usable mission object count : {count}").SetTextVariable("count", gameEntities.Count).ToString()));
                InformationManager.DisplayMessage(new InformationMessage(new TextObject("{=FeAe68E299}nearest usable mission object : {count}").SetTextVariable("count", reachableDroppedItem.Count).ToString()));
            }
